...
const Upload = () => {
  ...
  const onFormChange = (formattedData) => {
    ...
    if (formattedData.issnNumber && formattedData.issnNumber.length > 0) {
      filters.issnNumber = formattedData.issnNumber;
      hasSearchable = true;
    }
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "journals_template.xlsx");
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      const filename = hasActiveFilters ? "journals-filtered.xlsx" : "journals.xlsx";
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">
              Submit Journal Publication
            </h1>
            <p className="text-slate-400 mt-1">
              Enter details for a new journal publication record.
            </p>
          </div>
