...
const BulkImport = ({ isOpen, onClose, onSuccess }) => {
  ...
  const generateResultExcel = async () => {
    ...
    // Define headers
    const headers = [
      'Row #', 'Status', 'Error Message', 'Main Author', 'Email', 'Phone', 'Department',
      'Designation', 'Caste', 'Co-Authors', 'Title', 'Journal', 'Publisher', 'Year',
      'Volume', 'Issue Number', 'Pages', 'Indexation', 'ISSN Number', 'UGC Approved',
      'Impact Factor', 'Journal Link', 'Article Link'
    ];
    ...
    resultData.forEach((result) => {
      const row = worksheet.addRow([
        ...
        result.data['Volume'] || result.data['volume'] || '',
        result.data['Issue Number'] || result.data['issueNumber'] || '',
        result.data['Pages'] || result.data['pages'] || '',
        result.data['Indexation'] || result.data['indexation'] || '',
        result.data['ISSN Number'] || result.data['issnNumber'] || '',
        result.data['UGC Approved'] || result.data['ugcApproved'] || '',
        result.data['Impact Factor'] || result.data['impactFactor'] || '',
        result.data['Link to Journal'] || result.data['journalLink'] || '',
        result.data['Link to Article'] || result.data['articleLink'] || '',
      ]);
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          try {
            ...
            const payload = {
              ...
              year: row['Year'] || row['year'] ? parseInt(String(row['Year'] || row['year']).trim(), 10) : null,
              volume: row['Volume'] || row['volume'] || '',
              issueNumber: row['Issue Number'] || row['issueNumber'] || '',
              pages: row['Pages'] || row['pages'] || '',
              indexation: row['Indexation'] || row['indexation'] || '',
              issnNumber: row['ISSN Number'] || row['issnNumber'] || '',
              ugcApproved: row['UGC Approved'] || row['ugcApproved'] || '',
              impactFactor: row['Impact Factor'] || row['impactFactor'] ? parseFloat(String(row['Impact Factor'] || row['impactFactor'])) : null,
              journalLink: row['Link to Journal'] || row['journalLink'] || '',
              articleLink: row['Link to Article'] || row['articleLink'] || '',
            };

            // Validate required fields
            if (!payload.mainAuthor) throw new Error('Main Author is required');
            if (!payload.title) throw new Error('Title is required');
            if (!payload.journal) throw new Error('Journal name is required');
            if (!payload.year || isNaN(payload.year)) throw new Error('Year is required and must be a valid number');

            // Validate email
            if (!payload.email || payload.email.trim() === '') throw new Error('Email is required');
            if (!payload.email.includes('@') || payload.email.split('@').length !== 2) throw new Error('Invalid email format');
            ...
            <div className="p-6 space-y-6 relative">
              ...
              <div className="text-center space-y-4">
                <p className="text-sm text-slate-500">
                  Upload an Excel file (.xlsx) containing multiple journal publication records. Ensure column names match the standard format.
                </p>
