...
const PublicationsTable = forwardRef(({ showActions = false, externalFilters = {} }, ref) => {
  ...
  const fetchData = async () => {
    try {
      ...
      setError("Failed to load journal publications.");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    try {
      ...
      setDeleteId(null);
      toast.success("Journal publication deleted successfully");
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to delete journal publication");
    }
  };
  ...
  const columns = [
    ...
    { key: "email", label: "Email", minWidth: "160px" },
    { key: "phone", label: "Phone", minWidth: "130px" },
    { key: "department", label: "Department", minWidth: "120px" },
    { key: "designation", label: "Designation", minWidth: "180px" },
    { key: "caste", label: "Caste", minWidth: "80px" },
    { key: "coAuthors", label: "Co-Authors", minWidth: "180px" },
    { key: "title", label: "Title", minWidth: "280px" },
    { key: "journal", label: "Journal", minWidth: "200px" },
    { key: "publisher", label: "Publisher", minWidth: "150px" },
    { key: "year", label: "Year", minWidth: "80px" },
    { key: "volume", label: "Volume", minWidth: "80px" },
    { key: "issueNumber", label: "Issue No.", minWidth: "90px" },
    { key: "pages", label: "Pages", minWidth: "90px" },
    { key: "indexation", label: "Indexation", minWidth: "120px" },
    { key: "issnNumber", label: "ISSN Number", minWidth: "140px" },
    { key: "ugcApproved", label: "UGC Approved", minWidth: "120px" },
    { key: "impactFactor", label: "Impact Factor", minWidth: "120px" },
    { key: "journalLink", label: "Journal Link", isLink: true, minWidth: "140px" },
    { key: "articleLink", label: "Article Link", isLink: true, minWidth: "140px" },
  ];
                            ...
                            >
                              {col.key === 'email' && isOwnEntry ? (
                                ...
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                                  ...
                                  {row[col.key] === 'Yes' && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span>}
                                  {row[col.key] === 'No' && <span className="w-1.5 h-1.5 rounded-full bg-red-500 mr-1.5"></span>}
                                ...
                                <div className="flex items-center gap-2">
                                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm ${['bg-blue-500', 'bg-indigo-500', 'bg-purple-500', 'bg-pink-500', 'bg-teal-500', 'bg-orange-500'][row.mainAuthor?.length % 6] || 'bg-slate-500'
                                    }`}>
                                    {row.mainAuthor?.charAt(0) || '?'}
                                  </div>
                                  <span className="font-semibold text-slate-800">{row.mainAuthor}</span>
                                </div>
                              ) : (
                                row[col.key] || <span className="text-slate-300">-</span>
                              )}
                            ...
                            {isAuthenticated &&
                              ...
                              <>
                                <button
                                  ...
                                  title="Edit Entry"
                                  aria-label={`Edit publication ${row.title || row.issnNumber}`}
                                >
                                  <Edit size={16} />
                                </button>
                                <button
                                  ...
                                  title="Delete Entry"
                                  aria-label={`Delete publication ${row.title || row.issnNumber}`}
                                >
                                  <Trash2 size={16} />
                                </button>
                              </>
                ...
                <tr>
                  ...
                  >
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <h3 className="text-lg font-semibold text-slate-600 mb-1">No journal publications found</h3>
                      <p className="text-sm text-slate-500 max-w-xs mx-auto">
                        {Object.keys(filters).length > 0
                          ? "Try adjusting your search filters to find what you're looking for."
                          : "Be the first faculty member to submit a journal publication!"}
                      </p>
                      {Object.keys(filters).length > 0 && (
                        <button
                          onClick={() => setFilters({})}
