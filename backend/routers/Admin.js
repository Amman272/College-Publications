...
// NOTE: Journal publications use URL links — no local files to delete.
// This MUST be defined before DELETE /:id to prevent route conflict.
router.delete("/orphaned-files", verifyToken, requireSuperAdmin, async (req, res) => {
  return res.status(200).json({ message: "No files to delete (journal system uses URL links)", deleted: [], skipped: [] });
});
...
// Delete all journal publications (super admin only)
router.post("/deleteAll", verifyToken, requireSuperAdmin, async (req, res) => {
  try {
    // Wipe the journals table (no physical files in journal system)
    await db.query("DELETE FROM journals");
    await logAction(req.user.userEmail, "DELETE_ALL", "Deleted all journal publications");
    return res.status(200).json({ message: "All journal publications deleted" });
  } catch (err) {
    console.error(err);
...
router.get("/departments", verifyToken, requireAnyAdmin, async (req, res) => {
  try {
    // Get distinct departments from journals table
    const [departments] = await db.query(
      "SELECT DISTINCT department FROM journals WHERE department IS NOT NULL AND department != '' ORDER BY department"
    );
...
router.get("/stats", verifyToken, requireAnyAdmin, async (req, res) => {
  try {
    ...
    // Journal publication count (filtered by department for sub-admins)
    let journalQuery = "SELECT COUNT(*) as count FROM journals";
    let journalParams = [];
    if (!isSuperAdmin && dept) {
      journalQuery += " WHERE department = ?";
      journalParams = [dept];
    }
    const [[{ count: totalJournals }]] = await db.query(journalQuery, journalParams);
    ...
    if (isSuperAdmin) {
      const [[{ count: deptCount }]] = await db.query(
        "SELECT COUNT(DISTINCT department) as count FROM journals WHERE department IS NOT NULL AND department != ''"
      );
...
// NOTE: Journal publications use URL links, not local file uploads.
// This endpoint returns empty results as no physical files are managed.
router.get("/orphaned-files", verifyToken, requireSuperAdmin, async (req, res) => {
  return res.status(200).json({ orphaned: [], total: 0, orphanedCount: 0 });
});
